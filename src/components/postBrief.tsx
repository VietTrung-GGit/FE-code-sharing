...
const PostBrief: React.FC<PostBriefProps> = ({ postID}) => {
...
const currentUserId = 123;
 {/*  useEffect(() => {
    // Fetch post data from the backend when the component mounts
    const fetchPostData = async () => {
      try {
        const response = await axios.get(`/api/posts/${postID}`);
        setPost(response.data);
        setHasLiked(response.data.liked);
        setHasSaved(response.data.saved);
      } catch (error) {
...
*/}
 ...
 useEffect(() => {
    const fallbackPost: PostData = {
      postID: 1,
      userId: '123',
      displayName: 'I love you',  // Empty user name
      avatar: '',  // Empty avatar
      createat: '5/2/1',  // Empty date
      comments: 5,
      updateat: '',
      tags: [1, 2, 5],  // Tags available
      title: 'Sample Post with Missing Data',  // Title is there
      content: 'I love you',  // Empty content
      files: [
        { id: 1, name: 'file1.js', content: 'console.log("Hello, world!");' },
        { id: 2, name: '', content: '' },  // File with missing data
      ],
      likes: 3,
      liked: false,
      saved: true,
    ...
    setHasLiked(fallbackPost.liked);
    setHasSaved(fallbackPost.saved);
  }, []);
...
const handleMoreClick = () => {
  ...
  const resetPost = (timeupdate: string, newtitle: string, newcontent: string, newfiles: FileData[], newtags: number[]) => {
    if (post) {
    setPost({ ...post, updateat: timeupdate, title: newtitle, content: newcontent, files: newfiles, tags: newtags });}
  }
...
const handleSaveClick = () => handleSave(true);
  ...
  const handleDelete = async () => {
    ...
    if (confirmDelete) {
      ...
      try {
        ...
        // Optionally, redirect or remove the post from the UI
      } catch (error) {
        setVisible(true);
  ...
  const truncatedText = post?.content.length > 400 ? post.content.slice(0, 400) + '...' : post?.content;
  ...
  return (
    ...
    <div className="flex justify-center items-center relative">
      {visible && (
      <div className="bg-Background/Bottom text-white w-1/2 my-10 border-Primary/Dark border-2 rounded-3xl p-5 md:p-7 lg:p-8">
        {/* Avatar and Tags */}
      ...
      {/* Check if `updateat` is different from `createat` */}
        ...
        <div className="flex items-center">
          <p className="mb-4 w-full py-2 overflow-hidden">
            {truncatedText || 'No content available...'}
        ...
        <div>
          <Editor
            ...
            width="100%"
            language="javascript"
            value={post.files[activeTab]?.content || 'No content available'}
            theme="vs-dark"
            options={{
        ...
        <div className="flex justify-end space-x-4 mt-4">
          <button onClick={handleMoreClick} className="w-20 h-8 bg-white text-Primary/Dark inline-flex items-center justify-center py-2 px-4 rounded-lg">
            <svg
              className={`w-6 h-6 mr-2 stroke-current stroke-2`} // Tailwind class for color
              fill="none" // Use the current text color
              viewBox="0 0 24 24"
            >
              <path d="M12 21C16.9706 21 21 16.9706 21 12C21 7.02944 16.9706 3 12 3C7.02944 3 3 7.02944 3 12C3 13.4876 3.36093 14.891 4 16.1272L3 21L7.8728 20C9.10904 20.6391 10.5124 21 12 21Z" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            {post.comments}
          </button>
  ...
  />
      ...
      {showPostDetail && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex z-50">
           ...
           <div className='w-1/2 flex items-center'> <PostDetail proppost={post} toggleLike={handleLikeClick} toggleSave={handleSaveClick} propsaved={hasSaved} propliked={hasLiked} commentDelete={handleCommentChange}/> </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex z-50">
           ...
           <div className='w-1/2 flex items-center'> <PostCreate postData={post} postReset={resetPost}/> </div>
           <div className='w-1/4'> <button
