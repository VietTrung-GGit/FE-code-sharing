...
function Sidebar({ isOpen, state, onClose }: { isOpen: boolean; state: number; onClose: () => void }) {
  ...
  const [user, setUser] = useState<{ displayname: string; avatar: string }>({
    displayname: 'Loading...',
    avatar: 'https://cdn.pixabay.com/photo/2018/11/13/21/43/avatar-3814049_1280.png', // Default image until data is fetched
  });
      ...
      try {
        ...
        if (token) {
          ...
          setUser({
            ...
            avatar: response.data.avatar || '/defaultProfile.png',
          });
        }
      } catch (error) {
  ...
  <p className="text-white mt-6 sm:max-md:text-lg md:max-lg:text-base lg:max-xl:text-lg xl:max-2xl:text-xl">
    {user.displayname }
        ...
        <div className="flex flex-col mt-24">
          <button
            className={`m-2 flex items-center space-x-2 ${state === 1 ? 'text-green-500' : 'text-white'}`}
            onClick={() => handleNavigation('/community')}
          >
            <svg
              className={`${state === 1 ? 'stroke-green-500' : 'stroke-white'}`}
          ...
          <button
            className={`m-2 flex items-center space-x-2 ${state === 0 ? 'text-green-500' : 'text-white'}`}
            onClick={() => handleNavigation('/home')}
          >
            <svg
              className={`${state === 0 ? 'stroke-green-500' : 'stroke-white'}`}
              xmlns="http://www.w3.org/2000/svg"
            ...
            <span>Home</span>
                   ...
                   <button
            className={`m-2 flex items-center space-x-2 ${state === 2 ? 'text-green-500' : 'text-white'}`}
            onClick={() => handleNavigation('/saved')}
          >
            <svg
              className={`${state === 2 ? 'stroke-green-500' : 'stroke-white'}`}
              xmlns="http://www.w3.org/2000/svg"
 ...
 <path d="M2 5.29412C2 3.47483 3.64162 2 5.66667 2H20.3333C22.3584 2 24 3.47483 24 5.29412V30L13 20.1176L2 30V5.29412Z" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
          ...
          <button
            className={`m-2 flex items-center space-x-2 ${state === 3 ? 'text-green-500' : 'text-white'}`}
            onClick={() => handleNavigation('/profile')}
          >
            <svg
              className={`${state === 3 ? 'stroke-green-500' : 'stroke-white'}`}
              xmlns="http://www.w3.org/2000/svg"
