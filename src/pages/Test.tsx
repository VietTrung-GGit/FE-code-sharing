...
const Notifications = () => {
  const [notifications, setNotifications] = useState([
    ...
    { id: '2', message: 'Your post got 10 likes!', read: true },
    { id: '3', message: 'New comment on your photo!', read: false },
  ]);
  ...
  const toggleReadStatus = (id: string) => {
    ...
    // axios.patch(`/notifications/${id}`, { read: !notif.read });
  };
  ...
  return (
    <div>
      {notifications.map((notif) => (
        ...
        >
          {notif.message}
          <span className='ml-4 text-sm'>{notif.read ? 'Mark as Unread' : 'Mark as Read'}</span>
        </div>
      ))}
    </div>
  );
